#
# sockdiops-stat.awk for the DANTE socks 4/5 daemon
...
# uname -a | mail -s "I use sockdiops-stat!" eisvogel@hawo.stw.uni-erlangen.de
#
...
# http://www.hawo.stw.uni-erlangen.de/~eisvogel/sockdiops
#
#
# HOW TO USE
#
...
#
#    zcat /var/log/sockdiops.1.gz | awk -f /root/bin/sockdiops-stat.awk | \
...
/ sockdiops\[[0-9]+\]: block\([0-9]+\): / {
	...
	split($0,curr_line);
	for (i=1; i<=NF; i++)
		if (match(curr_line[i],/sockdiops\[[0-9]+\]:/)) {
			field_ofs = i + 1;
			break;
		}
	#
...
END {
	...
	CONVFMT="%d"
	r=rand()*999999999;
	tmpfile="/tmp/tmp.sockdiops-stat."r;
