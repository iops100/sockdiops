...
KNOWN_KEYWORDS="$LTINTERNAL|socks_conf|sockdiops_conf|pidfile|iomax|negmax|bufsize|libc|upnp|pam|bsdauth|full_env|gssapi_path|gssapi|krb5_config|krb5|krb5_path|ldap|ldap_path|sasl|sasl_path|glibc_secure|libwrap"
...
AC_ARG_WITH(sockdiops-conf,
[  --with-sockdiops-conf=FILE  change location of socks server configuration file],
[SOCKDCONFPATH="$withval"],
[#set default sockdiops.conf path
 SOCKDCONFPATH="/etc/${SERVNAME}.conf"])
AC_DEFINE_UNQUOTED(SOCKD_CONFIGFILE, "$SOCKDCONFPATH", [sockdiops config file])
