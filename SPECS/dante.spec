...
%description server
This package contains "sockdiops", the SOCKS proxy daemon and its
...
#
# processname: sockdiops
# config: %{_sysconfdir}/sockdiops.conf
# pidfile: %{_localstatedir}/run/sockdiops.pid
# # NOTE: if running with user.privileged as non-root, manually create
# #       pidfile owned by the user.privileged user before starting sockdiops.
# #       e.g., 'touch /var/run/sockdiops.pid; chown sockdiops /var/run/sockdiops.pid'
...
[ -x %{_sbindir}/sockdiops ] || exit 1
[ -r %{_sysconfdir}/sockdiops.conf ] || exit 1
...
%{__install} -m 0644 example/sockdiops.conf ${RPM_BUILD_ROOT}/%{_sysconfdir}
%{__install} -m 0755 sockdiops.init ${RPM_BUILD_ROOT}/%{_initrddir}/sockdiops
...
%post server
/sbin/chkconfig --add sockdiops

%preun server
if [ $1 = 0 ]; then
   /sbin/chkconfig --del sockdiops
fi

%files
%defattr(-, root, root, 0755)
%doc BUGS CREDITS NEWS README SUPPORT doc/README* example/socks.conf example/socks-simple-withoutnameserver.conf example/sockdiops.conf example/socks-simple.conf
...
%defattr(-, root, root, 0755)
%config %{_sysconfdir}/sockdiops.conf
%config %{_initrddir}/sockdiops
%{_sbindir}/sockdiops
%{_mandir}/man5/sockdiops.conf.5*
%{_mandir}/man8/sockdiops.8*
